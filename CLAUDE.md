...
- **Reuse the primitives** in `src/shared/ui/mono` (`MonoLabel`, `MonoCategory`, `MonoMetric`, `MonoStatusBadge`, `MonoDot`, `MonoMenu`, `MatrixText`) and the `.mono-*` CSS classes in `src/assets/styles/tailwind.css` (`.mono-label`, `.mono-cat`, `.mono-title`, `.mono-cta`, `.mono-btn-outline`, `.mono-icon-btn`, `.mono-field-label`, `.mono-input`). Don't copy-paste long class strings.
