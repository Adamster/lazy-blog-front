...
const initOf = (u: AuthorPostResponse) =>
...
export const PostViewMono = ({ post }: IProps) => {
  ...
  return (
    ...
    >
      ...
      <section className="mx-[calc(50%-50vw)] mt-10 w-screen bg-[var(--m-card)]">
        <div className="mx-auto flex max-w-[780px] flex-wrap items-end gap-x-4 gap-y-4 px-10 py-10">
          <Link
            ...
            className="font-display flex size-10 flex-none items-center justify-center self-center border-2 border-[var(--m-accent)] text-base font-bold text-[var(--m-accent)]"
          >
            {initOf(post.author)}
          </Link>
          <div className="min-w-0">
            ...
            <span className="text-[12px] text-[var(--m-muted)]">
              @{authorHandle} · {formatDate2(post.createdAtUtc)} · {readTime}{" "}
              min read
            </span>
          </div>
          ...
          <div className="ml-auto flex items-center gap-4 text-[12px] text-[var(--m-muted)] tabular-nums">
            ...
            <span className="inline-flex items-center gap-1.5">
              ...
              {(post.rating ?? 0).toLocaleString("ru-RU")}
            </span>
            <span className="inline-flex items-center gap-1.5">
      ...
      <article className="mx-auto max-w-[780px] px-10 pt-10">
        {/* Cover */}
        {post.coverUrl && (
          <div className="aspect-[16/9] w-full overflow-hidden border-2 border-[var(--m-dim)] bg-[var(--m-panel)]">
            <Image
              removeWrapper
              radius="none"
              src={post.coverUrl}
      ...
      {/* Comments */}
      <div className="mx-auto max-w-[780px] px-10 pb-10">
        <CommentsMono
          postId={post.id}
